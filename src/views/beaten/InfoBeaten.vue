<template>
  <div class="az-header">
    <br>
    <h2 class="az-content-title">Pre-processing List</h2>
            ...
            <table class="table table-striped mg-b-10">
              ...
              <tbody>
                <tr v-for="estate in estates" :key="estate.id">
                  ...
                  <td>{{estate.b_year}}</td>
                  <td>
                    <router-link :to="{path:`/beaten/table/${estate.id}`}" class="btn btn-info">Info</router-link>
                  </td>
                  <td>
                    <router-link :to="{path:`/beaten/${estate.id}`}" class="btn btn-primary">Edit</router-link>
                  </td>
                  <td>
...
export default {
  ...
  methods:{
    async fetchEstates(){
      let estatesSnapShot = await db.collection('beaten').where('estateId','==',this.estateId).get();
      let estates = [];
    ...
    async deleteEstate(estateId){
      if (confirm("Are you sure to delete this record?")) {
          let estateRef = doc(beatenColl, estateId);
          await deleteDoc(estateRef);
          alert('Beaten Deleted Successfully!');
          this.$router.go(-1);
      }
    }
  },
